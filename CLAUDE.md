...
- `apps/vol/`        — implied vol surface predictor. Numpy OLS over 10 surface-shape features (skew / smile / multi-horizon IV-HV ratio / OI imbalance / VIX-spread / strike-spread) → forward 20d IV/RV gap, top-quantile gated short-vol. v1 PASSES under per-rebal portfolio Sharpe (alpha +5.86 unrestricted), v2 #3 confirms OOS extension to 2026 (DoltHub proxy, 11/11 OOS quarters positive). Arc-decisive blocker: v2 #2 shows alpha collapses under deployable liquidity restriction (top-200 OI alpha −0.48). `partial-OOS` arc closure pending regime-gated-liquid-universe v3 test. Reuses `ss_iv` (loaders + short-vol PnL).

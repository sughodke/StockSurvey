...
- **Default any v1 of a partial-OOS predictor to a macro-gated deployment.** The 2026-05-10 macro regime diagnostic (`apps/gate/scripts/macro_regime_diagnostic.py`) pulled 6 FRED features at the val_start of each prediction-problem-pivot window (gate / pairs / vol = 17 windows total) and showed 5/6 features predict per-app-z-scored alpha (Pearson r `[+0.34, +0.49]` for credit_baa / fed_funds / vix / real_yield_10y; m2_yoy at −0.38; slope_10y_3m at noise). The single VIX-median split is the cleanest gate: **VIX > median → 5/8 wins (62%); VIX < median → 1/9 wins (11%)** — a 6× win-rate lift. Implementation: `packages/macro/` (`ss_macro`) wraps no-auth FRED CSV; cheapest macro-gated deployment is "only deploy the v0 predictor when recent VIX is above its 1y rolling median." Don't fold macro into `apps/regime` (that's per-ticker CWT-portfolio ranking; macro is universe-wide time-series — different aggregation levels). The natural home is `apps/gate` (already has the aggregate-features → exposure-gate contract). See `findings/macro-regime-diagnostic.md`.

...
- `apps/vol/`        — implied vol surface predictor. Numpy OLS over 10 surface-shape features → forward 20d IV/RV gap, top-K-gated short-vol on a 126d-VIX-rolling-median per-rebal regime gate composed with top-200 OI liquidity filter (v3 deployment recipe). v1 PASSES under per-rebal portfolio Sharpe (alpha +5.86 unrestricted), v2 #3 confirms OOS extension to 2026 (11/11 OOS quarters positive), v2 #2 showed alpha collapses under unconditional liquidity restriction (top-200 OI alpha −0.48), v3 rescues with regime gate (fired-α Sharpe +2.01 on 37% of rebals). Arc verdict: `partial-OOS` with v3 architecture as deployment recipe. The small-capacity re-frame (harvest the discarded illiquid VRP as a small operator) was tested 2026-05-15 and is `reversed-OOS` — the edge is real (+0.14/premium, artifact-cleaned) but 92.5% of the cohort is un-quotable on free data and the quotable 7.5% has negative edge; **do not re-attempt without a paid microcap option-quote source** (see `findings/vol-borrow-illiquid-vrp-falsified.md`). Live-infrastructure (`ss-vol live` + options broker) is unbuilt; DCA stays canonical live. Reuses `ss_iv` (loaders + short-vol PnL).
...
- `apps/cnc/`        — crypto-and-carry: perp funding-rate cash-and-carry basis trade. Numpy-only carry backtest on Hyperliquid top-20 perps (Binance/Bybit geo-blocked from this host, OKX exposes only 3mo public funding history). Loader `ss_loaders.hyperliquid` exposes funding-rate-history + perp 1d candles via the public HL `info` endpoint with on-disk parquet cache at `.hl-cache/`. v0 is `confirmed-OOS` vs locked pre-reg bar (net Sharpe +9.25 / DSR-t +11.8 / pos-Q 0.80 / max-DD −0.99% on 2024-01-01 → 2026-05-24) — but the headline is the academic-clean upper bound (zero basis-tracking error or short-spot borrow modeled), not a deployable Sharpe; 2024 fold Sh +13.8 carries most of the alpha and 2026YTD is Sh −0.63 as cross-coin funding compressed from 27.5%/yr → 0.4%/yr. See `findings/cnc-funding-rate-carry.md`.
- `apps/follow/`     — follow-the-leader: leadership-tier US congressional stock-disclosure follower. Long-only top-K basket of stocks recently *disclosed* as purchases by Speaker / party leaders / chair-or-ranking-member of Intelligence / Armed Services / Financial Services / Banking / Ways and Means / Finance / Appropriations (+ ≥10y-tenure heuristic backfill), entered at `filed + 1` trading day (the Bowne-2024 disclosure-lag-honest entry rule), equal-weight, 10 bps friction. Loader `ss_loaders.congress` exposes the Quiver-bundled `Congressional Trades.xlsx` + `unitedstates/congress-legislators` terms + a hand-curated 50-row leadership roster with cache at `.congress-cache/`. v0 is `confirmed-null` vs locked pre-reg bar — leadership-arm best-cell pooled-val ann Sh +0.86 / α vs SPY −0.09pp/yr / DSR-t −1.91; cross-arm surprise: all-members frequency-filter cell posts +1.01 Sh / +5.25pp/yr α (recorded as next-arc hypothesis, not pre-registered). See `findings/follow-leadership-disclosure.md`.
...
- `packages/loaders/`    (`ss_loaders`)    — Kaggle CSV matrix, Stooq archive, Yahoo, CryptoCompare, Hyperliquid perps/funding, symbol lists. Also `ss_loaders.congress` (Quiver-bundled congressional trades xlsx + Senate Stock Watcher JSON + `unitedstates/congress-legislators` terms + hand-curated leadership roster for `apps/follow`; on-disk cache at `.congress-cache/`).
...
- **Check for parallel/duplicate Modal apps before kicking off a new run** — `uvx modal app list | head -10` shows every ephemeral or running container. Multiple ephemeral apps with state=`ephemeral` and `Tasks >= 1` mean billable containers are running in parallel; each consumes GPU credit independently. Common causes of unwanted parallelism in this repo:
  - **Sub-agents and the parent session both invoking `uvx modal run`** — when an Agent is driving a Modal job in the background AND you (or another agent) also launch the same entrypoint manually, you double-burn. Before spawning a fresh `uvx modal run`, list apps and confirm no in-flight container is doing the same work.
  - **Retries after a transient failure** — a `uvx modal run` that errored locally may have left a still-alive remote container if the failure was post-`.remote()`-dispatch. The local CLI dropping doesn't always kill the remote. `uvx modal app stop ap-<id> --yes` is the surgical kill.
  - **Diagnostic loop**: (a) `uvx modal app list | head -15` to enumerate live apps; (b) `grep -o 'ap-[A-Za-z0-9]*' Output/<entrypoint>-run.log | head -1` to identify which app your own log is connected to; (c) stop any other live app for the same entrypoint with `uvx modal app stop ap-<id> --yes`.
- **For long runs, watch for idle/silent failure** — set a background watcher that polls every ~60s for (i) the expected `Output/<run>-results.json` materializing, (ii) the artifacts landing on the `ss-<app>-artifacts` Volume, (iii) the Modal app state going `stopped` with no results (silent failure → restart with corrections), (iv) N minutes of no log growth (idle → restart). The pattern is in `apps/docs/scripts/` and was used for the e2e_portfolio v3 recovery.
...
Operational rules extracted from individual findings live in their `apps/docs/docs/findings/*.md` page — each finding's lede is the actionable rule. The leaderboard row holds the evidence; the findings page holds the imperative. Don't restate rules here.
...
3. The actionable rule the arc established lives in the lede of its
   `findings/<topic>.md` page (and cross-links from related findings
   that reference it). The leaderboard row holds the evidence; the
   findings page holds the imperative. Don't duplicate it in
   `CLAUDE.md`.

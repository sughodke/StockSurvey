...
- `apps/dca/`        — multi-asset DCA. **Canonical live strategy as of 2026-05-13.** Fixed-target equal-weight basket on the 13-ETF Phase 4d universe (9 SPDR sector ETFs + TLT/IEF + GLD/DBC) at 1/13 each, quarterly rebal (80 trading-day cadence floor) with 5% per-name drift threshold for off-cadence triggers. 5 risk rails (kill-switch, freshness, cadence/drift gate, per-name cap, dry-run-by-default). Backtest 2005-2025: Sharpe +0.673, CAGR +8.4%, max DD −40.7% (GFC). Won the cfr-vs-dca-realistic friction comparison: CFR Phase 4d's raw +0.056 alpha collapsed to +0.015 net once realistic deployment costs were applied; DCA also won on worst-window Sharpe (+0.642 vs +0.434).
...
- **Any portfolio-level "win" must clear the passive equal-weight Sharpe on the same universe + window before being treated as shippable.** Use `alpha = model_val_sharpe − passive_val_sharpe` as the load-bearing column in any new leaderboard row that claims live-tradeability — raw val Sharpe is largely market beta of the chosen universe. The 2026-05-10 EW benchmark (`apps/relational/scripts/equal_weight_benchmark.py`, `Output/equal-weight-benchmark.json`) reclassified the three previously "shippable" relational rows: Phase-2 analog cross_ticker alpha = **+0.067** (≈ noise), stooq_us_long Morlet alpha = **−0.133**, ex-Phase-2 cross_ticker alpha = **−0.334**. No current checkpoint clears its passive baseline. See `findings/passive-ew-benchmark.md`. **2026-05-14 update:** the audit-flagged "ffill of trailing NaN inflates passive Sharpe by 0.05-0.15" caveat was empirically tested and falsified — Δ Sharpe under a strict-perm-death implementation is **≤ 0.0002** on every universe. The Stooq archive contains no permanently-delisted names in `phase-2`/`stooq_us_long`/`ex-phase-2`/`factor-wide` (manifest is hand-curated to survivors; Stooq archive itself reuses ticker symbols for replacement entities like BBBY → continuous $6-20 prices through 2025 despite the real-world bankruptcy in April 2023), so the benchmark's `ffill().dropna(axis=1)` is empirically a no-op. All alpha verdicts above stand as written. The real survivorship problem lives in manifest curation + ticker reuse, which the benchmark script cannot fix; this bias cancels in the alpha column (model and passive run on the same survivor cohort) but does inflate the absolute Sharpe-vs-zero benchmark. See `findings/delisting-aware-ew-falsified.md`.
...
- **Pair-spread mean reversion (`apps/pairs`) is regime-conditional, not unconditionally shippable on factor-narrow; the EG-passing-rate is NOT a usable regime gate.** The 2026-05-10 pairs classical v0 walk-forward (`apps/pairs/scripts/run_walkforward.py`) delivered mean agg val Sharpe **+0.099** across 6 windows (below the +0.20 fail floor pre-registered in `TODO/apps-pairs.md`), with 4/6 windows positive but a single catastrophic window (−1.23 in 2005-2007 bull market with dot-com-trained pairs). Mean ex-window-0 = +0.365 (would clear the marginal threshold) — pair trading worked in 2008-2017 mean-reverting markets, failed in trending bull markets. The v0 finding hypothesized that "EG-passing-rate per window is itself a regime indicator (high in mean-reverting, low in trending)" and the 2026-05-14 audit predicted gating on it would lift mean alpha +0.099 → ~+0.5. The 2026-05-14 follow-up (`apps/pairs/scripts/eg_gate_eval.py`, `findings/pairs-eg-gate-falsified.md`) **falsified this** at all three pre-registered thresholds (T_abs=3000 from audit, T_pct50=median of train EG counts=3320, T_pct30=30th-pct=2988): best-case gated alpha is +0.104 full-panel (+0.156 fired-only), 3-5× short of the predicted lift. **w0's train EG-pass count is 3918 — the third *highest* in the panel, not the lowest** — yet w0 posts the worst val Sharpe. Mechanism: train EG-pass count is dominated by cross-sectional cointegration density *during* the training slice (2000-2005 had high cointegration from dot-com crash forced co-movement); the relationships that produced the count broke OOS. Train-side regime indicators inherit the regime that produced the training data, not the regime that will receive the val deployment. Remaining v2 follow-ups untested: stop-loss on widening spreads, within-sector restriction, ML predictor. Speed: pass `maxlag=1` to `statsmodels.coint()` to skip BIC lag-selection inner loop (~17× faster, 500ms→30ms per pair). See `findings/pairs-classical-v0.md` and `findings/pairs-eg-gate-falsified.md`.
- **Vol surface signal lives in joint multivariate structure, not in any single feature — never pre-screen vol-surface features by univariate Pearson r.** The 2026-05-10 vol surface v0 walk-forward (`apps/vol/scripts/run_walkforward.py`) showed univariate Pearson r per feature ≤ +0.003 (the audit looked dead) but multivariate val Pearson r = **+0.12** (40× larger). The walk-forward wouldn't have run if we'd gated on univariate signal. This is the opposite of cross-sectional return prediction (where univariate ≈ multivariate IC at the +0.005 to +0.012 ceiling). Mean per-cell-Sharpe alpha +0.089 with 5/5 positive windows — `inconclusive` per pre-reg, partially refutes NO_OPTIONS.md's "the IV market efficiently incorporates the dislocation information" (true at ATM-IV level the prior arc tested; leaves residuals at the full-surface level the new feature class taps). Late windows (2022-12 → 2023-06) carry the signal: val r > +0.23. See `findings/vol-surface-v0.md`. **2026-05-14 update: v0's per-cell Sharpe was the bottleneck, not the signal.** The audit-driven v1 follow-up (`apps/vol/scripts/run_walkforward_v1.py`, `apps/vol/src/vol/portfolio.py`) replaced v0's pooled per-cell Sharpe with time-series annualized Sharpe of a 100-pick top-K basket rebal'd every 20 days, using alpha-vs-universe construction so friction cancels by symmetry. **Result: pooled alpha Sharpe +5.86, 5/5 positive, real-to-shuffle alpha-PnL ratio 25× ([`confirmed-OOS`](findings/vol-surface-v1.md)).** A cross-position correlation haircut (ρ≈0.3) drops the absolute Sharpe ~sqrt(5) ≈ 2.2× to ~2.6 in conservative dollar terms — still well above the +0.30 PASS threshold. **Operational rule: when a v0 finding's headline metric is flagged as "weak", run the honest metric before pivoting away from the workstream.** The +0.089 per-cell alpha had the right sign but understated temporal-consistent signal by an order of magnitude — pooling cells without temporal aggregation washed it out. **`apps/vol` is the most deployable workstream as of 2026-05-14**, contingent on (1) dollar-PnL conversion that preserves alpha under cross-position correlation and (2) an OI-based liquidity restriction (v2). DCA remains canonical *live* (simplest, no options friction); vol v1 is the active *research* workstream. MLP head deferred to v2 (linear already PASSES).
...
- **Memory-heavy regime gates (rolling multi-quarter medians) fight you across regime boundaries — a 1y-rolling-median criterion stays "calm" for ~12 months after a stress event because the median is inflated by the stress itself.** The 2026-05-13 final-swing experiment for the CFR arc (`apps/cfr/scripts/macro_gated_phase4d_eval.py`) composed Phase 4d with macro v1b's window-level VIX-above-1y-rolling-median gate. Pre-reg PASS = net alpha vs DCA ≥ +0.10 AND positive in ≥ 4/5 windows; pre-reg FAIL = alpha ≤ 0 OR positive in ≤ 2/5. **Result: alpha +0.053, positive in only 1/5 windows → FAIL (confirmed-null).** Mechanism: gate fired OPEN only in w4 (2022-07, post-Fed-pivot) where it correctly captured +0.263 alpha; held CLOSED in w0 (2010-03, post-GFC recovery) where it WRONGLY threw away CFR's best window (+0.422 raw alpha) because VIX 19 was below the 1y median of 25 (median inflated by GFC tail). The right-call save in w2 (avoided CFR's −0.508 calm-bull catastrophe) was offset by the wrong-call loss in w0 of nearly equal magnitude — net wash. **Three independent arcs converge: no current CFR configuration on this universe clears the +0.10 deployment threshold.** Operational rule for future gate compositions: any rolling-window regime gate must be tested for memory-window robustness across regime transitions; the gate that worked at +0.215 z lift on gate/pairs/vol's 6-window walk-forward did NOT transfer to Phase 4d's 5-window walk-forward because Phase 4d's windows happen to begin immediately after major stress events. **Note (2026-05-14):** the original "FAIL on positive in 1/5 windows" closure and the "CFR fully dead" framing were both overstated — the 2026-05-14 sensitivity follow-up (`findings/cfr-sensitivity-followup.md`) shows always-deploy CFR is 3/5 positive at every friction level and reclassifies the underlying alpha description as closer to `partial-OOS` than `confirmed-null` (alpha real, signed positive, 3/5 windows positive, below +0.10 deployment threshold). The deployment decision (DCA wins) is unchanged. DCA stays canonical live. See `findings/cfr-macro-gate-final.md` and `findings/cfr-sensitivity-followup.md`.
- **Backtest-positive alpha must survive realistic deployment friction before being treated as deployable; bracket the friction model, don't pin a single point estimate.** The 2026-05-13 cfr-vs-dca-realistic re-eval (`apps/cfr/scripts/plot_phase4d_vs_ew.py` + the inline friction model in `findings/cfr-vs-dca-realistic.md`) applied 50 bps/yr deployment drag to the CFR bot side and 5 bps/yr to the DCA side, producing net realistic alpha **+0.015**. The 2026-05-14 audit-driven sensitivity follow-up (`apps/cfr/scripts/sensitivity_analysis.py`) stress-tested both sides over the friction grid {10, 15, 25, 35, 50} × {2, 5, 10} bps/yr: **mean alpha swings 5× across the grid (+0.015 → +0.056)** but never crosses the +0.10 deployment threshold at any friction combination, with or without a gate. The single-point +0.015 figure is the worst-case corner; honest bracketed alpha is +0.015 to +0.056. **The positive-window count under always-deploy CFR is 3/5 across the entire friction grid, not 1/5** — the 1/5 in `cfr-macro-gate-final` was a denominator artifact (closed-gate windows contributed alpha=0 by construction). **Refined description of the CFR arc: alpha exists, signed positive, below +0.10 deployment threshold at every parameterization** (closer to `partial-OOS` than `confirmed-null` under the established verdict vocabulary; the sensitivity check itself is `diagnostic`) — alpha is real, signed positive, below deployment threshold at every parameterization tested. The deployment call (use DCA) is unchanged because nothing crosses +0.10; but the framing "bot is fully dead" was overstated. **Canonical live strategy is still `apps/dca`** — 13-asset EW basket, quarterly rebal, ~50 lines of Python. CFR research preserved as documented body of work for future regime-shift re-deployment. See `findings/cfr-vs-dca-realistic.md` and `findings/cfr-sensitivity-followup.md`.
- **Memory-heavy regime gates fight regime transitions, but no shorter-lookback fix has been found.** The 2026-05-13 `findings/cfr-macro-gate-final.md` diagnosis that the 252d-rolling-median VIX gate stays "calm" for ~12 months after a stress event (mis-classifying w0 2010-03 because VIX 19 < median 25, where the median was inflated by the GFC tail) is correct. The 2026-05-14 sensitivity follow-up tested four VIX lookbacks {60d, 90d, 126d, 252d} and one in-universe dispersion gate. **Result: 60d and 126d VIX gates never fire across any val_start in the panel; the 90d gate fires only in w3 (mean alpha +0.013, worse than baseline). The 252d gate, despite its memory-heavy flaw, is the least bad of the tested VIX lookbacks (+0.053 mean alpha).** The dispersion gate (60d/30d avg pairwise correlation among the 13 assets vs trailing 252d median) is **actively counter-productive — fires in exactly the wrong windows (w1/w2/w3, capturing w2's −0.508 alpha catastrophe) at mean alpha −0.115**. Mechanism guess: in 2010-03 and 2022-07 (CFR's good windows) cross-asset returns were aligned at a high level (recovery / inflation) so average pairwise correlation was high; in 2016-19 (CFR's worst window) within-equity decorrelation looked like dispersion but the cross-asset rotation alpha CFR needs wasn't there. Average pairwise correlation is the wrong dispersion measure for cross-asset rotation alpha. **Operational rule: when designing a regime gate for a cross-asset rotation strategy, the dispersion metric must isolate cross-bucket dispersion (e.g. corr between equity-bucket / bond-bucket / commodity-bucket aggregates), not average pairwise correlation across all constituents** — the latter is dominated by within-bucket structure which is not what the strategy trades. See `findings/cfr-sensitivity-followup.md`.

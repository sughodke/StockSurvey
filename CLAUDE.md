...
- **Memory-heavy regime gates (rolling multi-quarter medians) fight you across regime boundaries — a 1y-rolling-median criterion stays "calm" for ~12 months after a stress event because the median is inflated by the stress itself.** The 2026-05-13 final-swing experiment for the CFR arc (`apps/cfr/scripts/macro_gated_phase4d_eval.py`) composed Phase 4d with macro v1b's window-level VIX-above-1y-rolling-median gate. Pre-reg PASS = net alpha vs DCA ≥ +0.10 AND positive in ≥ 4/5 windows; pre-reg FAIL = alpha ≤ 0 OR positive in ≤ 2/5. **Result: alpha +0.053, positive in only 1/5 windows → FAIL (confirmed-null).** Mechanism: gate fired OPEN only in w4 (2022-07, post-Fed-pivot) where it correctly captured +0.263 alpha; held CLOSED in w0 (2010-03, post-GFC recovery) where it WRONGLY threw away CFR's best window (+0.422 raw alpha) because VIX 19 was below the 1y median of 25 (median inflated by GFC tail). The right-call save in w2 (avoided CFR's −0.508 calm-bull catastrophe) was offset by the wrong-call loss in w0 of nearly equal magnitude — net wash. **Three independent arcs now converge on the same answer: no current CFR configuration on this universe survives realistic deployment.** Operational rule for future gate compositions: any rolling-window regime gate must be tested for memory-window robustness across regime transitions; the gate that worked at +0.215 z lift on gate/pairs/vol's 6-window walk-forward did NOT transfer to Phase 4d's 5-window walk-forward because Phase 4d's windows happen to begin immediately after major stress events. CFR arc fully closed; DCA stays canonical live. See `findings/cfr-macro-gate-final.md`.
- **Backtest-positive alpha must survive realistic deployment friction before being treated as deployable.** The 2026-05-13 cfr-vs-dca-realistic re-eval (`apps/cfr/scripts/plot_phase4d_vs_ew.py` + the inline friction model in `findings/cfr-vs-dca-realistic.md`) applied 50 bps/yr deployment drag to the CFR bot side (ETF spread slippage on TLT/DBC, short-term tax inefficiency from ~11× annual one-way turnover, operational labor monitoring kill-switches / tax-lot reconciliation / annual model retrain) and 5 bps/yr to the DCA side (quarterly rebal × ~50% turnover, ETF in-kind redemption tax-efficient). Net per-window Sharpe: CFR **+0.815** vs DCA **+0.800** → realistic alpha **+0.015** (3.7× shrinkage from raw +0.056). On a $100k portfolio that's ~$200/yr extra return — comfortably exceeded by the operational labor of running the bot. **DCA also wins on worst-window Sharpe by +0.21** (CFR worst window +0.434 vs DCA +0.642 in w2 2016-19 calm-bull regime). Across all 9 CFR phase variants tested 2026-05-12 there is no current configuration that survives realistic friction. **Operational rule: any backtest-positive strategy must be re-scored with explicit friction (slippage, tax wrapper, operational labor) before paper trading.** Phase 4d reclassified from `PASS` (raw) to `confirmed-null` (realistic). **Canonical live strategy is now `apps/dca`** — 13-asset EW basket, ~50 lines of Python, no AI/Modal/checkpoint refresh. CFR research preserved as documented body of work for future regime-shift re-deployment. See `findings/cfr-vs-dca-realistic.md`.

...
| [`cfr`](cfr.md) | Active (**Phase 4d PASS** 2026-05-12) | Deep CFR meta-allocator across existing strategy menu. **Nine phase variants shipped same day.** Phases 1-3 (tabular → deep CFR on US equities) hit a +0.02 architectural ceiling. **Phase 4 universe-shift sweep (4 variants): 4a VIX-bar-gate FAILS (−0.30 alpha), 4b sector ETFs partial-OOS (first positive alpha vs EW at +0.015), 4c 5-day rebal partial-OOS (friction-bound), 4d 13-asset multi-asset PASSES** — mean CFR +0.861, CFR vs naive +0.101 (clears +0.10 cut), mean alpha vs passive EW **+0.056** (positive for the first time across all 9 variants), 3/5 positive alpha windows. **The arc-level lesson:** meta-allocator is prediction-problem bound, not representation bound — iterating on architecture hit +0.02 ceiling, iterating on universe broke through it. Phase 5 = build `ss-cfr live` and paper trade Phase 4d. |

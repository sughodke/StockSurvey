...
| [`cfr`](cfr.md) | Active (Phase 2 confirmed-null 2026-05-12) | Deep CFR meta-allocator across existing strategy menu. Tabular CFR over `(infoset, action)`. **Phase 1 (16 actions): CFR ties naive uniform mix (+0.002 Sharpe Δ).** **Phase 2a (28 actions, +4 documented-alpha modes): CFR loses to naive uniform by 0.06.** **Phase 2b (31 actions, +real SEC 13F-HR consensus from new `packages/edgar`): CFR loses to naive uniform by 0.07; but window 5 alone — the only one with full 13F coverage in train+val — posts CFR alpha +0.277 vs Phase 1, validating that 13F has signal where it has data.** Binding constraint is tabular sample density, not menu content. Phase 3 = deep CFR with `regret_net(state, action_emb)` + learned encoder. |

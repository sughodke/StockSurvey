...
| [`cfr`](cfr.md) | Active (Phase 3 MARGINAL 2026-05-12) | Deep CFR meta-allocator across existing strategy menu. Five phase variants shipped same day. **Phase 1 (tabular, 16 actions): CFR ties naive uniform mix.** **Phase 2a (28 actions, +4 documented-alpha modes) and Phase 2b (31 actions, +real SEC 13F-HR consensus from new `packages/edgar`): both confirmed-null on tabular menu enrichment.** **Phase 3 (Deep CFR with tinygrad regret_net + 10-feature continuous state vector incl. 4 FRED macro features): MARGINAL — mean CFR +0.614 (best of all phases), CFR alpha vs EW −0.071 (32% improvement over Phase 1's −0.093), window 2 flips −0.111 → +0.127.** Cumulative Phase 1 → 3 lift is +0.021 — far short of +0.15 PASS floor. Binding constraint is no longer architecture or menu — it's signal availability in the menu × universe × horizon combination. Phase 4 should change the prediction problem (different universe / horizon), not the meta-allocator's representation. |
...
| [`cfr`](cfr.md) | **Phase 1-2: Tabular CFR over `(infoset, action)`** — infoset is `(vol-bucket, dispersion-bucket)` 3×3 = 9 cells; action menus 16 → 28 → 31 (Phase 1 / 2a / 2b adding documented-alpha + real SEC 13F-HR consensus modes). **Phase 3: Deep CFR** — replaces the table with a tinygrad MLP `regret_net(state_vec) → R^31` over a 10-feature continuous state vector (6 universe-internal + 4 FRED macro features), AdamW lr=5e-4 wd=1e-3, expected-baseline regret variance reduction. Same 31-action menu as Phase 2b. | `cfr/menu.py` + `cfr/state.py` + `cfr/tabular.py` (Phase 1-2) ; `cfr/state_vec.py` + `cfr/deep.py` + `cfr/deep_walkforward.py` (Phase 3) |

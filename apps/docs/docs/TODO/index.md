...
- [Meta-allocator with strategy-internal features](meta-allocator-internal-features.md) — pre-reg locks orthogonal lever (input space, not model class) after [`meta-allocator-regime-forecasting`](../findings/meta-allocator-regime-forecasting.md) `confirmed-null` for 5 macro-feature learners. Feature set: per-arc valuation spreads (DCA basket P/E vs 10y; vol_v3 IV-vs-RV gap percentile; relational fingerprint dispersion; pairs z-score; gate trailing DD) + per-arc 252d rank-IC trend + cross-arc correlation effective rank, **all at quarterly cadence** (Haddad-Kozak-Santosh 2020 + Macrosynergy). 3-model grid {ridge, RF-depth-3, 2-PC predictive regression}. Verdict bar `confirmed-OOS` requires fold-3 OOS ΔSR ≥ +0.30 vs B3 AND Ledoit-Wolf CI excludes 0 AND DSR-t > +3.0; bootstrap CI > ±0.40 auto-downgrades. Closes the meta-allocator arc if null (every reasonable feature class tested).

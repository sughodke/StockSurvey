"""Modal entrypoint for e2e-portfolio v3.6 rolling-retrain walk-forward.

Same architecture as v3.5 (one universe + 4-output model + long-vol head),
but the train-eval loop is the ZZR-style continuous rolling walk:
  - Initial 5y train ending before the earliest val_start
  - Walk val span forward in `retrain_every` (default 63d = quarterly) chunks
  - Eval each chunk with current weights, then fine-tune 500 steps from
    those weights as the rolling window slides forward
  - Single model walks through all 3 folds (continuous, default) OR resets
    per fold (academic-CV mode, opt-in via --per-fold-reset)

Reuses v3.5 prep pickle (`e2e-portfolio-v3p5-prep.pkl` on ss-e2e-iv-data
Volume) since the underlying panel is identical.

Run:
  uvx modal run --detach apps/e2e_portfolio/scripts/modal/train_v3p6_walkforward.py \\
      --initial-steps 5000 --refine-steps 500 --retrain-every 63
"""
...
app = modal.App('e2e-portfolio-v3p6-walkforward', image=image)
...
def train_v3p6_walkforward_remote(
    vol_npz: bytes,
    phase4d_pkl: bytes,
    v3_pooled_npz: bytes,
    initial_steps: int,
    ...
    # Reuse v3.5 prep pickle (same panel, same VIXY data).
    iv_vol_prep = iv_vol / 'e2e-portfolio-v3p5-prep.pkl'
    if not iv_vol_prep.exists():
        raise RuntimeError(
            'v3.5 prep not found on ss-e2e-iv-data volume; upload first via '
            '`uvx modal volume put ss-e2e-iv-data '
    ...
    print(f'  loaded v3.5 prep from volume ({len(prep_pkl) // (1 << 20)} MB) '
          f'— v3.6 reuses identical panel + VIXY', flush=True)
    ...
    print('\n=== Step 2/3: load v3.5 panel + rolling-retrain walk ===', flush=True)
    import pickle, json
    import numpy as np
    import pandas as pd
    ...
    from e2e_portfolio.eval_v3p6 import (
        FOLDS, run_fold_rolling, run_walkforward_continuous, pool_and_report,
    )
    ...
    print(f'  mode: {"PER-FOLD RESET" if per_fold_reset else "CONTINUOUS (default)"}',
          flush=True)
    ...
    if per_fold_reset:
        ...
        for fold_cfg in FOLDS:
            res = run_fold_rolling(
                full_panel, close, vixy_close, fold_cfg, cfg, hp,
                initial_steps=initial_steps, refine_steps=refine_steps,
                retrain_every=retrain_every,
                save_prefix='e2e-portfolio-v3p6-perfold')
            per_fold.append(res)
        save_prefix = 'e2e-portfolio-v3p6-perfold'
    else:
        per_fold = run_walkforward_continuous(
            full_panel, close, vixy_close, FOLDS, cfg, hp,
            initial_steps=initial_steps, refine_steps=refine_steps,
            retrain_every=retrain_every,
            save_prefix='e2e-portfolio-v3p6')
        save_prefix = 'e2e-portfolio-v3p6'
    print(f'  walk done in {time.perf_counter() - t0:.0f}s', flush=True)

    print('\n=== Step 3/3: pool + baseline comparisons ===', flush=True)
    summary = pool_and_report(per_fold, save_prefix=save_prefix)
    results_path = output / f'{save_prefix}-results.json'
    ...
    for p in sorted(output.iterdir()):
        if p.is_file() and p.name.startswith(save_prefix):
            data = p.read_bytes()
...
def walkforward(
    ...
    mode = 'PER-FOLD-RESET' if per_fold_reset else 'CONTINUOUS'
    print(f'launching e2e-portfolio v3.6 walkforward on Modal '
          f'[mode={mode}] (initial_steps={initial_steps} '
          f'refine_steps={refine_steps} retrain_every={retrain_every}d)')
    artifacts = train_v3p6_walkforward_remote.remote(

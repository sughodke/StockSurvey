"""Live orchestration for the relational scorers: checkpoint + broker
-> rebalance.

Mirrors `regime/live.py` end-to-end (same four risk rails, same
broker surface, same `LiveRunResult` shape). Only the inference path
differs — `relational.inference.target_weights` instead of
`regime.inference.target_weights`.
...
def run_live(
    ...
    *,
    ...
    """
    cp: RelationalCheckpoint = load_checkpoint(checkpoint_path)
    ...
    if ks.exists():
        return LiveRunResult(
            ...
            strategy=cp.strategy, dry_run=dry_run, account=account,
            last_bar_date='', n_universe=0,
            target_weights=pd.Series(dtype=float), trades=[],
            aborted_reason=f'kill-switch present at {ks}')

    # Same wavelet-support requirement as regime/live.py — see comment
    # there. All relational strategies use the CWT, so scales is
    # always non-empty (no rsi-style zero-extent fallback path).
...
def format_run(result: LiveRunResult) -> str:
    ...
    lines = [
        f'relational live @ {result.timestamp}',
...
__all__ = ['DEFAULT_KILLSWITCH', 'LiveRunResult', 'run_live', 'format_run']

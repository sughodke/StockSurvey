"""Modal entrypoint — Phase 4c deep CFR with 5-day rebal cadence.

Same Phase 3 architecture, same Phase 2b 31-action menu, same
universe (stooq_us_long 312 tickers). Only `rebal_days=5` instead
of 20 — 4× more samples for SGD per window, 4× more friction.

Pre-registered cut: mean CFR Sharpe ≥ Phase 3 + 0.10 AND alpha vs
EW-rebal-5 baseline ≥ +0.05.

Caveat: 5-day rebal at 10 bps round-trip = ~5% friction/year (vs
Phase 3's ~1.3%/year at 20-day). The trade-off: more SGD samples
might let the regret_net find a finer regime structure, but the
friction tax has to be paid out of any signal it finds.
"""
...
app = modal.App('cfr-phase4c', image=image)
...
def run_walkforward_remote(
    ...
    *,
    ...
    print('\n=== Step 3/4: build menu ===', flush=True)
    ...
    print(f'  rebal_days={rebal_days} (Phase 3 used 20)', flush=True)
    ...
    payload = {
        'config': {
            'phase': '4c', 'rebal_days': rebal_days,
            'train_window_days': train_window_days,
            'val_window_days': val_window_days,
            'step_window_days': step_window_days,
...
def main(
    ...
    rebal_days:        int = 5,        # ← the only Phase 3 → 4c change
    top_k:             int = 20,
    hidden:            int = 64,
    learning_rate:     float = 5e-4,
    batch_size:        int = 64,
    train_every:       int = 10,       # 4× rebals → 2× train_every keeps SGD wall similar
    n_sgd_per_batch:   int = 5,
    seed:              int = 0,
    filing_lag_days:   int = 45,
    output_name:       str = 'cfr-phase4c.json',
) -> None:

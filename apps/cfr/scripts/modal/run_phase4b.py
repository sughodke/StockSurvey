"""Modal entrypoint — Phase 4b deep CFR on sector ETF universe.

10 SPDR sector ETFs (XLB/XLE/XLF/XLI/XLK/XLP/XLU/XLV/XLY) + SPY,
2005-2025 (~5y less span than Phase 3 since SPDRs started 2005).
Same Phase 3 deep CFR architecture; smaller `top_k` for the modes
(3 instead of 20) since the universe is 10 names.

`Top13FConsensusMode` is dropped from the menu (13F filings are
about individual stocks, not sector ETFs) — Phase 4b menu is the
Phase 2a 9-mode set × {0, 0.5, 1.0, 2.0} gross = 28 actions.

Pre-registered cut: mean CFR Sharpe ≥ Phase 3 + 0.20 AND alpha vs
EW-of-sectors (i.e., SPY-like equal-weight rebalanced) ≥ +0.15.
"""
...
app = modal.App('cfr-phase4b', image=image)
...
def run_walkforward_remote(
    ...
    *,
    ...
    print('\n=== Step 3/4: build menu (no 13F) + state-vec ===', flush=True)
    ...
    print(f'  Phase 4b menu ({menu.n_actions} actions) on {close.shape[1]}-ETF universe',
          flush=True)
    ...
    payload = {
        'config': {
            'phase': '4b',
            'menu': 'phase2a (no 13F) on sector ETFs',
            'train_window_days': train_window_days,
            'val_window_days': val_window_days,
            'step_window_days': step_window_days,
            'rebal_days': rebal_days,
...
def main(
    ...
    rebal_days:        int = 20,
    top_k:             int = 3,    # smaller universe → smaller top-K
    hidden:            int = 64,
    ...
    seed:              int = 0,
    output_name:       str = 'cfr-phase4b.json',
) -> None:
    LOCAL_OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    close_path = LOCAL_OUTPUT_DIR / 'cfr_phase4b_sector_close.pkl'

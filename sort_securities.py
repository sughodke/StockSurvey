...
tickers_lookup['stocks'] = tickers

tickers = """
ANT,ARDR,ARK,BCC,BCH,BLOCK,BTC,BTCD,CVC,DASH,DASH,FUN,GBYTE,ICO,MAID,MYST,NLC2,NXS,PART,PAY,PIVX,PPT,
QTUM,RDD,SYS,VERI,VIA,WAVES,WINGS,XEM,XVG
"""
tickers = tickers.replace('\n', '').replace(' ', '').split(',')
tickers_lookup['coins'] = list(map(lambda x: 'coin{}'.format(x), tickers))
tickers_lookup['both'] = tickers_lookup['coins'] + tickers_lookup['stocks']
...
def value_security(ticker):
    ...
    return (0.4 * np.mean(r1[-5:]) + 0.6 * np.mean(r2[-5:]))/2


def sortby_relevance(only_names=False, limit=200, key='n100'):
    """ weight=(0.4, 0.6) """
    t = tickers_lookup[key]
    # with Pool(5) as p:
    #     val = p.map(value_security, t)
    val = map(value_security, t)

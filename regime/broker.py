...
Wraps `alpaca-py` to expose the four operations `live.py` needs:
account snapshot, current positions, recent OHLC bars for a universe,
and submitting orders to reach a target dollar allocation.
...
class AlpacaBroker:
    ...
    def __init__(
        ...
        self._paper = base_url == PAPER_BASE_URL

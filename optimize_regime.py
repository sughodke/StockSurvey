"""
optimize_regime : walk-forward hyperparameter optimization for the regime
ranking strategy using Optuna.

Splits the backtest period into train/validate/test windows:
  - Optuna searches params on the train window
  - Best params are validated on the next window
  - Final eval on held-out test window

Usage:
    uv run python optimize_regime.py --data-dir ./Nasdaq3347
    uv run python optimize_regime.py --data-dir ./Nasdaq3347 --n-trials 200
    uv run python optimize_regime.py --data-dir ./Nasdaq3347 --objective calmar
"""
...
# All candidate scales (trial picks a subset)
ALL_SCALES = [3, 5, 7, 10, 12, 15, 21, 26, 42, 50, 63, 90, 126]


def weights_regime_parameterized(prices, lookback, n_tail, top_n, scales,
                                  divergence='kl', spread_df=None, max_spread=0.02):
    """Regime strategy with tunable parameters."""
    ...
    if spread_df is not None:
        ...
        for i in range(scores.shape[0]):
            illiquid = spread_arr[i + lookback] > max_spread
...
def run_backtest(prices, weight_df, rebalance_days, commission_bps=10):
    """Run a single bt backtest, return result stats."""
    from backtest_bt import make_commission_fn

    rebal_weights = weight_df.iloc[::rebalance_days]

    # Align price columns with weight columns
...
def extract_metric(result, metric):
    """Extract a scalar metric from bt result."""
    stats = result.stats
    if metric == 'sharpe':
        return float(stats.loc['daily_sharpe', 'regime'])
    elif metric == 'calmar':
        return float(stats.loc['calmar', 'regime'])
    elif metric == 'cagr':
        return float(stats.loc['cagr', 'regime'])
    elif metric == 'sortino':
        return float(stats.loc['daily_sortino', 'regime'])
    return 0.0
...
def make_objective(prices_train, rebalance_days, metric, commission_bps=10,
    ...
    """Create an Optuna objective function closed over the training data."""
...
def _resolve_scales(params):
    """Build scales list from trial params."""
    scales = []
...
def walk_forward_optimize(prices, n_trials, rebalance_days, metric,
    ...
    For each window:
      1. Train: optimize params on train_years (with transaction costs)
      2. Validate: test best params on val_years
      3. Step forward by step_years and repeat

    Returns list of (window, best_params, train_score, val_score).
    """
    ...
    while True:
        ...
        window_label = f'{window_start.date()} → {train_end.date()} | {val_end.date()}'
        print(f'\n{"=" * 70}')
        ...
        scales = _resolve_scales(best)

        try:
            weight_df = weights_regime_parameterized(
                prices_val, best['lookback'], best['n_tail'], best['top_n'],
                scales, divergence=best['divergence'],
...
def plot_optimization_results(wf_results, metric):
    ...
    ax = axes[1]
    param_names = ['lookback', 'n_tail', 'top_n']
...
if __name__ == '__main__':
    ...
    parser.add_argument('--n-trials', type=int, default=50,
                        help='Optuna trials per window (default: 50)')
    ...
    parser.add_argument('--step-years', type=int, default=2,
                        help='Step forward between windows in years (default: 2)')
    ...
    parser.add_argument('--max-spread', type=float, default=0.02,
                        help='Max Corwin-Schultz spread to include (default: 0.02 = 2%%)')
    ...
    # Check which params are stable across windows
    print(f'\nParameter stability:')
    for pname in ['lookback', 'n_tail', 'top_n', 'divergence']:
        ...
        if isinstance(vals[0], (int, float)):
            ...
            counts = Counter(vals)
            print(f'  {pname}: {dict(counts)}')

    fig = plot_optimization_results(wf_results, args.objective)
    if args.save:
        fname = 'Output/optimize-regime-walkforward.png'
